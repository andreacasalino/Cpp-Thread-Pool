...
// Samples are drawn within the square [0,1] x [0,1] and the number of samples inside the unitary radius circle centred in (0,0) is computed
...
public:
    ...
    double evalPi() { return 4.0 * (double)this->Trials_inside / (double)this->Trials_tot; };
...
// serial version
{
    ...
    Simulation.newSimulation(NUMBER_OF_SIMULATIONS * TRIAL_X_SIMULATION); //all the samples are drawn with a single call 
...
//pool of threads
{
    ...
    for (unsigned long k = 0; k < NUMBER_OF_SIMULATIONS; ++k)
        pool.push([&Simulation]() { Simulation.newSimulation(TRIAL_X_SIMULATION); });
